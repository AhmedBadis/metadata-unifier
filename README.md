...
This project, developed for my bachelor thesis, explores the integration of artificial intelligence (AI) in research systems to improve data management and evaluation processes. It leverages large language models (LLMs) and natural language processing (NLP) techniques to enhance the extraction, transformation, and cleaning of research data, facilitating more efficient and scalable workflows.
...
The core goal of this project is to implement an AI-powered system that automates and optimizes metadata cleaning, data formatting, and analysis across different research platforms. By leveraging NLPs such as the spaCy module in python and LLMs such as **Qwen2.5-1.5B-Instruct** and combining them with imperative programming techniques, the system can import research data from different platforms such as Zotero, PyRAT and RSpace then preprocess it and convert it into a unified structured JSON format for easier interpretation and further analysis in the interdisciplinary field of digital medicine.
...
- Improve the efficiency of managing and analyzing large datasets in research.
...
- **Python**: The primary programming language used for implementation, including various libraries such as `spaCy`, `requests`, and `zotero`.
...
- **NFDI4Health Integration**: Deploys transformed data directly to the NFDI4Health database, with full compatibility via Local Data Hub (ldh-deployment).
...
3. **Integrating Research Data into NFDI4Health's LDH**:
   To integrate the transformed metadata into the NFDI4Health Local Data Hub (LDH), run idh-deployment/start.sh.
...
### License
   ...
   For more details on the LDH, please refer to the other README.md within the ldh-deployment directory.